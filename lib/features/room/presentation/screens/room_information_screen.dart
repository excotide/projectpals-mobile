...
import 'room_owner_edit_screen.dart';
...
class _RoomInformationScreenState extends State<RoomInformationScreen> {
  ...
  void initState() {
    super.initState();
    _room = widget.room;
    // Non-owner di-redirect ke RoomMemberScreen (lihat build); tak perlu
    // memuat daftar member untuk layar member.
    if (_isOwner) {
  ...
  /// Jalankan smart matching (owner, status open). Syarat (§FLOWS 1.1)
  /// divalidasi backend; pesan 422 ditampilkan lewat listener RoomFailure.
  void _startMatching() {
    if (_matching) return;
    setState(() => _matching = true);
    context.read<RoomBloc>().add(RoomMatchRequested(_room.roomCode));
  }
  ...
  List<MemberEntity> get _filteredMembers {
    ...
    return _members
        .where(
          (m) =>
              (m.user?.name ?? '').toLowerCase().contains(
                _memberSearch.toLowerCase(),
              ) ||
              (m.primaryRole ?? '').toLowerCase().contains(
                _memberSearch.toLowerCase(),
              ),
        )
        .toList();
  ...
  Widget build(BuildContext context) {
    // Validasi kepemilikan: non-owner → layar detail member (FLOWS §2).
    if (!_isOwner) {
      return RoomMemberScreen(room: _room);
    }
    return BlocListener<RoomBloc, RoomState>(
      listener: (context, state) {
        ...
        } else if (state is RoomMatched) {
          // Match sukses → room jadi ongoing; sembunyikan kartu Smart Matching.
          setState(() {
            _matching = false;
            _room = _room.copyWith(status: 'ongoing');
          });
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Teams formed successfully!'),
                      ...
                      _RoomInfoCard(
                        ...
                        onEditTap: () {
                          Navigator.push(
                            context,
                            MaterialPageRoute(
                              builder: (_) => RoomOwnerEditScreen(room: _room),
                            ),
                          );
                        },
                      ),
                      // Smart matching hanya untuk owner saat room masih open.
                      if (_isOwner && _room.status == 'open') ...[
  ...
  void _confirmDelete() {
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        content: const Text(
          'Room ini akan dihapus permanen.',
  ...
  void _confirmLeave() {
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        content: const Text(
          'Anda yakin ingin keluar dari room ini?',
          style: TextStyle(color: Colors.white60, fontSize: 14),
        ),
        actions: [
          TextButton(
      ...
      child: Column(
        children: [
          ...
          Padding(
            ...
            child: Column(
              children: [
                _RoomCodeRow(roomCode: room.roomCode),
...
// ── Room Code row (dengan tombol salin) ────────────────────────────────────────

class _RoomCodeRow extends StatelessWidget {
  final String roomCode;
  const _RoomCodeRow({required this.roomCode});
